public class parcial8 {
    ...
    public static void procesarMatriz(int[][] matriz) {
        int fila = 0;
        while (fila < MAXFILA) {
            int reordenamientos = OrdenarASClasDESC(matriz[fila]);
            System.out.println("Fila " + (fila + 1) + ": " + reordenamientos + " reordenamientos");
            if (reordenamientos >= X) {
                System.out.println(">> En el estante " + (fila + 1) + " hubo al menos " + X + " reordenamientos");
            }
            fila++;
        }
    }
